...
pub fn decode_polyglot_move(board: &Board, raw_data: u16) -> MoveData {
    ...
    let move_type = if promote_move_type.is_some() {
        if let Some(capture_piece) = board.squares[to as usize] {
            PromotionCapture(PromotionCaptureStruct {
                ...
                promoted_piece: Piece::new(board.turn, promote_move_type.unwrap()),
            })
        } else {
            MoveType::Promotion(Piece::new(board.turn, promote_move_type.unwrap()))
        }
    } else if let Some(capture_piece) = board.squares[to as usize] {
        if board.squares[from as usize].unwrap().piece_type == PieceType::PAWN
            && board.game_state.en_passant_square.is_some()
            && board.game_state.en_passant_square.unwrap() == to
        {
        ...
        MoveType::Castling(CastlingMove {
            side: CastlingSide::get_castling_from_squares(from, to, board.turn).unwrap(),
            color: board.turn,
        })
    } else {
        MoveType::Normal
    ...
    MoveData {
        ...
        to: to,
        move_type: move_type,
        piece_to_move: board.squares[from as usize].unwrap(),
    }
}
