...
use crate::engine::movegen::precomputed::{ALIGN_MASK, DIR_RAY_MASK, NUM_SQUARES_FROM_SQUARE, SQR_A_B_MASK};
...
pub fn get_attacking_pieces(board: &Board, square: u8, piece_color: PieceColor) -> Bitboard {
    ...
    attackers |= movegen::precomputed::KNIGHT_MOVES[square as usize] & board.get_piece_bitboard(piece_color.opposite(), PieceType::KNIGHT);
    ...
    attackers |= precomputed::KING_MOVES[square as usize] & board.get_piece_bitboard(piece_color.opposite(), PieceType::KING);
...
pub fn get_attackers_vec(board: &Board, square: u8, piece_color: PieceColor) -> Vec<(Piece,u8)> {
    ...
    // Check for knight attacks
    let mut knight_attackers = precomputed::KNIGHT_MOVES[square as usize] & board.get_piece_bitboard(piece_color, PieceType::KNIGHT);
...
pub fn generate_knight_move(board: &Board, move_list: &mut MoveList,only_captures:bool) {
    ...
    while *knights != 0  {
        let from_sqr=knights.pop_lsb();
        let mut moves=precomputed::KNIGHT_MOVES[from_sqr as usize] & !blockers & board.check_ray & quiet_bitboard;
...
pub fn generate_king_move(board: &Board, move_list: &mut MoveList,only_captures:bool) {
    ...
    let quiet_bitboard = if only_captures { opp_pieces } else { Bitboard::new(u64::MAX) };
    let from_sqr = kings.pop_lsb();
    let mut moves = precomputed::KING_MOVES[from_sqr as usize] & !blockers & !board.attacked_square & quiet_bitboard;
    let mut captures = moves & opp_pieces;
    moves &= !captures;
